...
/**
 * Normaliza un número de teléfono mexicano para WhatsApp.
 * Devuelve el número en formato E.164 sin el "+" (para usar en wa.me):
 *   10 dígitos  → 521XXXXXXXXXX
 *   11 dígitos (1+10) → 52XXXXXXXXXXX
 *   12 dígitos (52+10) → 521XXXXXXXXXX
 *   13 dígitos (521+10) → ya correcto
 */
export function normalizarTelefonoMX(telefono) {
  if (!telefono) return null
  const d = telefono.replace(/\D/g, '')
  if (d.length === 10)                          return `521${d}`
  if (d.length === 11 && d.startsWith('1'))     return `52${d}`
  if (d.length === 12 && d.startsWith('52'))    return `521${d.slice(2)}`
  if (d.length === 13 && d.startsWith('521'))   return d
  if (d.length === 13 && d.startsWith('52'))    return d
